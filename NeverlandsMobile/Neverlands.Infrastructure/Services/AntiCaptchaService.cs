...
public class AntiCaptchaService : IAntiCaptchaService {
    ...
    public async Task<int?> CreateTaskAsync(byte[] imageBytes) {
        ...
        var requestObj = new { clientKey = _clientKey, task = new { type = "ImageToTextTask", body = body, phrase = false, @case = false, numeric = 1, math = false, minLength = 5, maxLength = 5 } };
        ...
        try {
            ...
            if (result != null && result["errorId"]?.Value<int>() == 0) return result["taskId"]?.Value<int>();
            return null;
        } catch { return null; }
    }
    public async Task<JObject?> GetTaskResultAsync(int taskId) {
        var requestObj = new { clientKey = _clientKey, taskId = taskId };
        var json = JsonConvert.SerializeObject(requestObj);
        var content = new StringContent(json, Encoding.UTF8, "application/json");
        try {
    ...
    public async Task<string?> SolveCaptchaAsync(byte[] imageBytes) {
        ...
        for (int i = 0; i < 60; i++) {
            ...
            if (result["errorId"]?.Value<int>() != 0) return null;
            var status = result["status"]?.Value<string>();
            if (status == "ready") return result["solution"]?["text"]?.Value<string>();
